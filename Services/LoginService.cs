    ...
    public class LoginService
    {
        ...
        public static async Task<ApiResponse<AccessToken>> AccessToken(string username, string password, string scope)//Get access token ofa user by passing his clientId and SecretId
        {
            ...
            try
            {
                ...
                {
                    Config.UserInfos.accessTokenApiView = accessToken.Content.access_token;
                }
                if (scope == "apim:api_create")
                {
                    Config.UserInfos.accessTokenCreate = accessToken.Content.access_token;
                }
                if (scope == "apim:api_publish")
                {
                    Config.UserInfos.accessTokenPublish = accessToken.Content.access_token;
                }
                if (scope == "apim:subscription_view")
                {
                    Config.UserInfos.accessTokenSubscriptionView = accessToken.Content.access_token;
                }
                if (scope == "apim:subscription_block")
                {
                    Config.UserInfos.accessTokenSubscriptionBlock = accessToken.Content.access_token;
                }
